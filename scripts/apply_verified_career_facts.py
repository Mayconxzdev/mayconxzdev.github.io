...
PT_PROOF = '''<section class="proof-strip" aria-labelledby="proof-title">
          <div class="section-heading section-heading--compact"><p>PROVAS EM CONTEXTO</p><h2 id="proof-title">Números que apontam para sistemas em uso - não para marketing.</h2><span>Cada indicador pertence ao sistema ou ambiente que o sustenta. Valores operacionais podem evoluir com o uso.</span></div>
          <ol class="metric-list"><li><a class="metric-link" href="competencias/" aria-label="Abrir competências: mais de 10 mil execuções na instância n8n"><strong>10 mil+</strong><span>execuções na instância n8n</span><small>múltiplas automações em produção · abrir evidências ↗</small></a></li><li><a class="metric-link" href="cases/helpdesk/" aria-label="Abrir case HelpDesk: 11 usuários"><strong>11</strong><span>usuários no HelpDesk</span><small>uso interno real · abrir case ↗</small></a></li><li><a class="metric-link" href="cases/producao-operacional/" aria-label="Abrir Produção Operacional: mais de 20 profissionais"><strong>20+</strong><span>profissionais apoiados na produção</span><small>10+ PCs · 1 TV · 9 setores · abrir case ↗</small></a></li><li><a class="metric-link" href="cases/mala-direta/" aria-label="Abrir Mala Direta: seis campanhas"><strong>6</strong><span>campanhas operacionais</span><small>base de 1.020 contatos · uma com 900+ ↗</small></a></li><li><a class="metric-link" href="cases/vesper-propostas/" aria-label="Abrir Vesper Propostas: menos de 30 segundos"><strong>&lt; 30s</strong><span>em propostas simples</span><small>antes: 2 a 4 minutos · abrir case ↗</small></a></li><li><a class="metric-link" href="cases/catalogo-operacional-compras/" aria-label="Abrir Catálogo Operacional: mais de 480 códigos"><strong>480+</strong><span>códigos no Catálogo Operacional</span><small>24 categorias · uso diário · abrir case ↗</small></a></li></ol>
        </section>'''
...
EN_PROOF = '''<section class="proof-strip" aria-labelledby="proof-title">
          ...
          <ol class="metric-list"><li><a class="metric-link" href="skills/" aria-label="Open skills evidence: more than 10 thousand executions in the n8n environment"><strong>10k+</strong><span>executions in the n8n environment</span><small>multiple production automations · open evidence ↗</small></a></li><li><a class="metric-link" href="cases/helpdesk/" aria-label="Open HelpDesk case: 11 users"><strong>11</strong><span>HelpDesk users</span><small>real internal use · open case ↗</small></a></li><li><a class="metric-link" href="cases/producao-operacional/" aria-label="Open Produção Operacional: more than 20 professionals"><strong>20+</strong><span>professionals supported in production</span><small>10+ PCs · 1 TV · 9 areas · open case ↗</small></a></li><li><a class="metric-link" href="cases/mala-direta/" aria-label="Open Mala Direta: six campaigns"><strong>6</strong><span>operational campaigns</span><small>1,020-contact base · one with 900+ ↗</small></a></li><li><a class="metric-link" href="cases/vesper-propostas/" aria-label="Open Vesper Propostas case: under 30 seconds"><strong>&lt; 30s</strong><span>for simple proposals</span><small>before: 2 to 4 minutes · open case ↗</small></a></li><li><a class="metric-link" href="cases/operational-procurement-catalog/" aria-label="Open Operational Procurement Catalog: more than 480 codes"><strong>480+</strong><span>codes in the procurement catalog</span><small>24 categories · daily use · open case ↗</small></a></li></ol>
        </section>'''
...
def replace_section(text: str, class_name: str, replacement: str, label: str) -> str:
    ...
    if replacement in text:
        return text
    raise RuntimeError(f"{label}: expected one section {class_name}, found {count}")


def remove_archive_row(text: str, number: str, label: str) -> str:
    ...
    if count == 1:
        return updated
    if f'<span class="archive-number">{number}</span>' not in text:
        return text
    raise RuntimeError(f"{label}: could not remove archive row {number}")
...
def save(relative: str, text: str) -> None:
    ...
    if text != original:
        ...
        print(f"verified facts applied: {relative}")
    else:
        print(f"verified facts already current: {relative}")
...
def update_home(relative: str, *, english: bool) -> None:
    ...
    label = relative
    text = replace_section(text, "proof-strip", EN_PROOF if english else PT_PROOF, label)
    text = replace_section(text, "experience", EN_EXPERIENCE if english else PT_EXPERIENCE, label)
    ...
    if english:
        ...
        replacements = [
            ...
            ('"Text-to-video"', '"Geração de mídia","AWS","PySpark"'),
        ]

    for old, new in replacements:
        text = replace_once(text, old, new, label)
    text = remove_archive_row(text, "16", label)
    save(relative, text)
...
def update_cases() -> None:
    cases: dict[str, list[tuple[str, str]]] = {
        ...
        "en/cases/mala-direta/index.html": [
        ...
        "cases/producao-operacional/index.html": [
            ...
            ("A evidência pública não contém credenciais, dados pessoais ou informações internas sensíveis.", "A implantação interna cobre escritório e chão de fábrica; a publicação utiliza apenas dados fictícios e não expõe caminhos, documentos, banco operacional ou infraestrutura da empresa."),
        ],
        "en/cases/producao-operacional/index.html": [
            ...
            ("The public evidence contains no credentials, personal data or sensitive internal information.", "The internal deployment covers office and factory-floor use; the public release uses fictional data and exposes no paths, documents, operational database or company infrastructure."),
        ],
        ...
        "en/cases/compass/index.html": [
            ...
            ("<article><span>04</span><h2>What was demonstrated</h2><ul><li>Program completed with 10 sprints.</li></ul></article>", "<article><span>04</span><h2>What was demonstrated</h2><ul><li>Ten sprints completed between Oct. 2024 and Mar. 2025.</li><li>Public pipeline with CSV/API ingestion, S3, Lambda, Glue/PySpark, Parquet, Athena and QuickSight.</li><li>Historical learning experience, not presented as a production environment.</li></ul></article>"),
        ],
        "cases/helpdesk/index.html": [
            ("Portal Vesper", "Portal"),
        ],
        "en/cases/helpdesk/index.html": [
            ("Portal Vesper", "Portal"),
        ],
    }
...
def main() -> None:
    update_home("index.html", english=False)
    update_home("en/index.html", english=True)
    update_skills("competencias/index.html", english=False)
    update_skills("en/skills/index.html", english=True)
    update_cases()
