...
try{
  ...
  if(expanded!==14)failures.push(`PT mobile: expected all 14 secondary projects after expand, got ${expanded}`);
  ...
  if(filtered<1||filtered>=14)failures.push(`PT mobile: process filter returned suspicious row count ${filtered}`);
  ...
  if(await visibleRows(en)!==14)failures.push(`EN mobile: expected 14 secondary projects after expand, got ${await visibleRows(en)}`);
  ...
  if(await visibleRows(desktop)!==14)failures.push(`Desktop: expected all 14 secondary projects, got ${await visibleRows(desktop)}`);
}finally{
  await browser.close();
}
