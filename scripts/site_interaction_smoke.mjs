...
try{
  ...
  if(expanded!==12)failures.push(`PT mobile: expected all 12 secondary projects after expand, got ${expanded}`);
  ...
  if(filtered<1||filtered>=12)failures.push(`PT mobile: process filter returned suspicious row count ${filtered}`);
  ...
  if(await visibleRows(en)!==12)failures.push(`EN mobile: expected 12 secondary projects after expand, got ${await visibleRows(en)}`);
  ...
  if(await visibleRows(desktop)!==12)failures.push(`Desktop: expected all 12 secondary projects, got ${await visibleRows(desktop)}`);
}finally{
  await browser.close();
}
